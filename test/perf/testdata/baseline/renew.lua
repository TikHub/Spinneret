...
local nxs = sp_int_str(nx)
redis.call('HSET', lkey, 'x', nxs)
redis.call('ZADD', base .. 'lsexp', nxs, lid)
if sp_num(v[5], 0) == 1 and v[7] then
  local idkey = base .. 'id:' .. v[7]
  local id = redis.call('HMGET', idkey, 'st', 'xl')
  if id[1] then
    if nx > sp_num(id[2], 0) then
      redis.call('HSET', idkey, 'xl', nxs)
    end
    redis.call('ZADD', base .. 'rdy:' .. (v[6] or ''), 'XX', 'GT', nxs, v[7])
  end
end
return {'OK', nxs, v[9] or '', v[6] or '', v[10] or '', v[11] or '', v[12] or '', v[13] or '0'}
