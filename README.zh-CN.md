...
四十台机器共用一池十万个会话，而池子的状态没有任何地方完整记着。

具体是什么样子：周末吞吐掉了一半，什么都没崩。四十个 worker 从同一个 Redis 集合里捞 cookie，其中四个账号
周六晚上被封了、到现在还在被发出去，而这吃掉了多少请求没人算得出来。某个代理网段从凌晨两点开始返回验证码
页，到底是代理坏了还是账号坏了，只有恰好抽到这一对的那台 worker 上的一行日志知道。想改一下轮换间隔，得重新
部署 40 个容器。

池子是共用的，关于池子的信息不是。

Spinneret 把这份状态收到一个地方。每个节点按请求申请，拿到一个凭据和一个出口，然后上报结果。池子只有一份
状态，也只有一个归属。
...
| 一个账号的每个请求都从同一个出口出去 | 代理模式 `bind_identity`，配上 `rebind_tolerance` 和 `max_rebinds_per_day`，免得一条线路故障变成「同一个账号一下午从三个国家发出请求」 |
...
一个账号被封了，另外三十九台机器还不知道，于是它们继续用它，对面继续从这些尝试里学到东西。一个封号就是这样
变成好几个的。
...
**动作是有影响范围的。** 范围开大了，一个坏账号会把四十个好账号一起停掉。一个动作落在六种作用域之一：
