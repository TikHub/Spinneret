...
这个除法用的注册表和分片那套是同一类，但有一处刻意的不同：每个 API 实例每 2 秒心跳一次，心跳存活窗口是
**60 秒**，每次心跳都重新除一遍。这里留了 30 个心跳的余量，而 worker 分片注册表只留 5 个——因为写心跳本身
要用 Redis，而这个闸门起作用的时刻恰恰就是 Redis 被打满的时刻。用 10 秒窗口实测过：繁忙的那个实例漏掉了
足够多的心跳，被对端从存活列表里剔除，对端于是把集群预算除以 1、把全部额度都放了出去——在最该收紧的时刻
反而把闸门开大，正好喂给了这个闸门本该阻止的崩塌。加长窗口的代价是：一个崩掉的实例会继续占着它那份额度，
最多一分钟。默认编排两个副本、预算 64，每个准入 32。扩到四个，每个准入 16 —— 集群总量不变。一次尝试最多等
