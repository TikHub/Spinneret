...
**这些在控制台的「设置 → 系统」页面也能改。** 那里其实更合适：改动在下一次每小时的任务时生效，不用重启，
而且不可能像环境变量那样在各副本之间不一致。两个来源只朝一个方向合并——**这里设了变量，变量就赢，并且把
这项钉死**，控制台会把它显示为只读，并告诉你要去掉哪个变量才能接管它。本仓库发布的 `.env` 一个都不写，
所以正常部署下它们全都从默认值开始，由控制台管理。控制台还多出两项根本没有对应变量的保留期：告警历史
（以前固定 90 天）和下面那个 ClickHouse TTL。
...
原始请求事件在 ClickHouse 里，按 `SPINNERET_CLICKHOUSE_TTL_DAYS` 过期，和这五个完全独立。告警事件默认按
90 天窗口清理，现在也能在控制台里和其余几项一起设置。
