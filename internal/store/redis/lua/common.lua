...
--@sp sp_hs_raw
--[[
sp_hs_raw(s) -> score, sts, samples, nfail, lastfail, cd, ru, lu (raw strings)
        or nil when s is absent, empty or not exactly eight "|" fields.

The eight fields of a packed identity x endpoint-group state (spec §5.1) as
they stand on the wire, without a table and without a single tonumber. A caller
that rewrites some fields and leaves the others alone pays for the fields it
really uses: it parses only those, re-formats only those, and splices the rest
back as the strings they already are. Lua 5.1 parses a two-decimal score in
0.45 us and a 13-digit millisecond timestamp in 0.07 us with sp_dnum, so not
touching "cd", "ru" and "lu" is worth about 1 us per rewrite on its own.

nil means "this entry is not in the current eight-field form": the caller falls
back to sp_hs_unpack, which supplies the defaults for a missing entry and the
short forms written by older releases.
]]
local function sp_hs_raw(s)
  if s == nil or s == false or s == '' then
    return nil
  end
