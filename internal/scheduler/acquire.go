...
// acquireWithWait runs acquire.lua, retrying exhausted and no-proxy results
// until the wait budget is spent (spec §6.1 step 4).
func (s *Service) acquireWithWait(ctx context.Context, p *authz.Principal, t *target, count int, session string, wait time.Duration) (acquireOutcome, error) {
	...
	for attempt := 0; ; attempt++ {
		out, err := s.execAcquire(ctx, p, t, count, session)
		if err != nil {
			if ctxErr := ctx.Err(); ctxErr != nil {
				return out, ctxErr
			}
			return out, apperr.Internal(fmt.Errorf("acquire %s/%s: %w", t.site.ID, t.group.ID, err))
		}
		...
		delay := waitDelays[min(attempt, len(waitDelays)-1)]
		if delay > remaining {
			delay = remaining
		}
...
// execAcquire performs one acquire.lua call.
func (s *Service) execAcquire(ctx context.Context, p *authz.Principal, t *target, count int, session string) (acquireOutcome, error) {
	call := acquireCall{
		now:      s.clock(),
		count:    count,
		session:  session,
	...
	if err != nil {
		return acquireOutcome{}, fmt.Errorf("run acquire script: %w", err)
	}
	return parseAcquire(vals)
}
