...
if pmode ~= 'n' then
select_proxy = function(region, exclude)
    ...
    for _, p in ipairs(ids) do
      if p ~= exclude and not seen[p] then
        ...
        if h[1] ~= 'active' then
          ...
          if ok and kinds and not kinds[h[2] or ''] then ok = false end
          if ok and providers and not providers[h[4] or ''] then ok = false end
          if ok and regions and not regions[h[3] or ''] then ok = false end
          ...
          if ok then
            local s = sp_decay(sp_num(h[8], baseline), sp_num(h[9], now), now, baseline, tau)
            if s < 5 then s = 5 end
