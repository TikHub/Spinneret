...
local bucket_ms = tonumber(ARGV[3]) or 5000
local buckets = tonumber(ARGV[4]) or 12
local min_requests = tonumber(ARGV[5]) or 1
local risk_gte = tonumber(ARGV[6]) or 0
local captcha_gte = tonumber(ARGV[7]) or 0
local success_lte = tonumber(ARGV[8]) or 0
local open_ms = tonumber(ARGV[9]) or 120000
local max_open_ms = tonumber(ARGV[10]) or 3600000
local reset_after_ms = tonumber(ARGV[11]) or 1800000
local close_min = tonumber(ARGV[12]) or 5
local close_ratio = tonumber(ARGV[13]) or 0.8
...
-- Read positionally instead of through sp_hmget_map: a twelve-entry field
-- array and a twelve-entry result table are two allocations this script does
-- not need.
local cur = redis.call('HMGET', bkey,
  'st', 'ou', 'oc', 'lo', 'lc', 'man', 'hw', 'hc', 'ps', 'pk', 'rsn', 'v')

local st = cur[1]
if st ~= 'open' and st ~= 'half_open' then st = 'closed' end
local ou = sp_num(cur[2], 0)
local oc = sp_num(cur[3], 0)
local lo = sp_num(cur[4], 0)
local lc = sp_num(cur[5], 0)
local man = sp_num(cur[6], 0)
local hw = sp_num(cur[7], 0)
local hc = sp_num(cur[8], 0)
local ps = sp_num(cur[9], 0)
local pk = sp_num(cur[10], 0)
local rsn = cur[11] or ''
local v = sp_num(cur[12], 0)
...
local nh = 0
for b = first, last do
  -- b is an integral index of at most ten digits, which Lua concatenates with
  -- "%.14g", exactly its digits; the counters are small, so tonumber is used
  -- directly rather than through sp_num (36 calls over a twelve-bucket window).
  local suffix = eg .. ':' .. b
  local vals = redis.call('HMGET', base .. 'win:' .. suffix, 't', 's', 'r')
  total = total + (tonumber(vals[1]) or 0)
  success = success + (tonumber(vals[2]) or 0)
  risk = risk + (tonumber(vals[3]) or 0)
  nh = nh + 1
  hlls[nh] = base .. 'winh:' .. suffix
end
local captcha = 0
if #hlls > 0 then
...
-- Only an evaluation can transition, so mode "read" creates none of the four
-- closures below (about 0.4 us each on Valkey 8.1).
local open_for, ratio_str, go_closed, go_open
if mode == 'eval' then
  ...
  ratio_str = function(x)
    return string.format('%.2f', x)
  end

  go_closed = function(reason, trig)
  ...
  go_open = function(reason, trig, next_oc)
  ...
  if to ~= '' or lazy == 1 then
    redis.call('HSET', bkey,
      'st', st, 'ou', ou, 'oc', oc, 'lo', lo, 'lc', lc,
      'man', man, 'hw', hw, 'hc', hc, 'ps', ps,
      'pk', pk, 'rsn', rsn, 'v', v)
  end
