...
// a single row carrying a large tag field must fail fast, not burn CPU
// proportionally to n^2. With 300k distinct tokens the old O(n^2) dedup took
// well over a minute; the cap now short-circuits after MaxTags distinct values
// while scanning lazily, so it returns near-instantly. The generous bound
// (relative to the pre-fix cost) keeps the test non-flaky under -race and on
// loaded CI while still failing on any quadratic regression.
func TestImportTagsDoSBounded(t *testing.T) {
	t.Parallel()
	huge := buildTags(300_000, ",")
	...
	require.Empty(t, rows)
	require.Len(t, rowErrs, 1)
	require.Less(t, elapsed, 10*time.Second, "tag processing must be bounded, got %v", elapsed)
}
